...
public class Exercises {
	...
	public boolean firstLast6(int[] nums) {
		...
		outputVariable = (nums[0] == 6 || nums[nums.length - 1] == 6) ? true : false;
	...
	public boolean sameFirstLast(int[] nums) {
		...
		arrayLength = (nums.length >= 1) && (nums[0] == nums[nums.length -1]) ? true : false;
	...
	public boolean commonEnd(int[] a, int[] b) {
		...
		boolean commonEndCheck;
		
		commonEndCheck = (a[0] == b [0] || a[a.length -1] == b[b.length -1]) ? true : false;
		
		return commonEndCheck;
	}
