  ...
  computed: {
    filteredList(){
      ...
      if(this.filter.firstName !== ""){
        ...
        return entry.firstName.toLowerCase().includes(this.filter.firstName.toLowerCase()); 
        });
      }
      if(this.filter.lastName !== ""){
        output = output.filter((entry)=>{
        return entry.lastName.toLowerCase().includes(this.filter.lastName.toLowerCase()); 
        });
      }
      if(this.filter.username !== ""){
        output = output.filter((entry)=>{
        return entry.username.toLowerCase().includes(this.filter.username.toLowerCase()); 
        });
      }
      if(this.filter.emailAddress !== ""){
        output = output.filter((entry)=>{
        return entry.emailAddress.toLowerCase().includes(this.filter.emailAddress.toLowerCase()); 
        });
      }
      if(this.filter.status !== ""){
        output = output.filter((entry)=>{
        return entry.status === this.filter.status; 
        });
      }
      return output;
